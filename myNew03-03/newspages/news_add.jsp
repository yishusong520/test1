...
<div id="main">
  ...
  <div id="opt_area">
    ...
    <form action="${pageContext.request.contextPath}/AdminSerlet?action=addnew" method="post" enctype="multipart/form-data" onsubmit="return check()">
      <p>
        <label> 主题 </label>
        <select name="ntid">
	        <!-- 迭代输出主题 -->
	        <c:forEach var="top" items="${top }">
	        	<option value='${top.tid}'>${top.tname}</option>
	        	</c:forEach>
	        <!-- 迭代输出主题 -->
        </select>
      </p>
